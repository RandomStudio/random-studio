...
const ImageContainer = ({ currentCoordIndex, isPlaying }, ref) => {
  ...
  const currentImages = useMemo(() => {
    if (!isPlaying) return currentCoordIndex === 0 ? [gpsData[currentCoordIndex].images[0]] : [gpsData[currentCoordIndex].images[1]];
  ...
  return (
    <section className={styles.container}>
      {imageSprings.map(({ opacity }, index) => {
        ...
        const {
          placeholder, src, srcSet, isLoaded,
        } = imagesWithState[index];
